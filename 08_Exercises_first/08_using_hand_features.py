...
def findMaxCountors(countors):
    ...
    for i in range(len(countors)):
        area_hand = cv2.contourArea(countors[i])
...
while 1:
    ...
    if len(countors) > 0:  # En az bir kontür algılanırsa
        ...
        cv2.line(roi, extRight, extLeft, (255, 0, 0), 2)  # Mavi çizgi
        ...
        b = math.sqrt((extTop[0]-extLeft[0])**2+(extTop[1]-extLeft[1])**2)  # Sağ yan kenar uzunluğu
        c = math.sqrt((extRight[0]-extLeft[0])**2+(extRight[1]-extLeft[1])**2)  # Hipotenüs uzunluğu
